...
class KuDig(BaseProfile):
    ...
    def _load_template(self, template_file):
        return Environment(loader=FileSystemLoader(join(TEMPLATES_DIR, "kudig"))).get_template(template_file)
    ...
    def _make_diff(self):
        try:
            diff_template = self._load_template("diff." + self.outputformat)
            diff_text = ""

            with open("diff", "r") as f:
              diff_text = "<br>".join(f.readlines())

        except:
            diff_text = "No diff file found"

        return diff_template.render(
            diff_text = diff_text
        )

    def _make_validation_report(self):
        try:
            valrep_template = self._load_template("validation_report." + self.outputformat)
            report = ""

            with open("validation-report.ttl", "r") as f:
              report = "<br>".join(f.readlines())
        except:
            report = "No validation report file found"
    ...
    def _make_document(self):
        ...
        return self._load_template("document." + self.outputformat).render(
            ...
            validation_report = self._make_validation_report()
        )
